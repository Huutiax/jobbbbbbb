              ...
              <el-form :model="form" size="mini" label-width="70px">
                ...
                <div>
                  ...
                  <el-button
                    type="primary"
                    size="mini"
                    @click="openErpEbayDevPage"
                    >产品开发页</el-button
                  >
                  <el-button
                    type="primary"
                    size="mini"
                    @click="openErp289ReportPage"
                    >289报表</el-button
                  >
                </div>
              </el-form>
...
export default {
  ...
  methods: {
    ...
    /**
     * @description 打开开发ebay产品开发页面
     * ① 传入所有itemId，逗号组装
     * ② 开发单状态为初审
     * @see https://erp.bz-bss.com/ebay/develop-product-spu/index
     */
    openErpEbayDevPage() {
      const keywordsStr = this.items.join(",");

      window.open(
        `https://erp.bz-bss.com/ebay/develop-product-spu/index?DevelopProductSpuSearch[item_id]=${keywordsStr}&DevelopProductSpuSearch[map_status][]=7`
      );
    },
    /**
     * @description 打开289报表页面，注意：该报表有权限，只有jack和少数人能访问
     * ① 传入所有itemId，逗号组装
     * ② 更新日期为本月，即开始日期为本月1号就好
     * @sese https://erp.bz-bss.com/pivot/view?id=289
     */
    openErp289ReportPage() {
      const keywordsStr = this.items.join(",");
      const date = new Date();
      const currentMonth = `${date.getMonth() + 1}`.padStart(2, "0");
      const currentMonthStartDate = `${date.getFullYear()}-${currentMonth}-01`;

      window.open(
        `https://erp.bz-bss.com/pivot/view?id=289&match[item_id]=${keywordsStr}&match[updated_at][0]=${currentMonthStartDate}&sort=7@total_sold`
      );
    },
    /**
     * @description 批量打开关键字30天tp销量页面
     * https://erp.bz-bss.com/ebay/develop-product-spu/index?DevelopProductSpuSearch[item_id]=373694304579,384248017729&DevelopProductSpuSearch[map_status][]=7
     */
    batchOpenTp30() {
