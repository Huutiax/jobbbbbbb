<template>
  <DatePicker
    ...
    :class="[`app-datepicker--${options.size || 'small'}`]"
  ...
  >
    ...
    <div slot="footer" class="app-datepicker__footer">
      <div class="quick-date-list">
        <AppTag
          v-for="(item, i) in options.range ? ranges : quickDateList"
          :key="i"
          @click.native.stop="quickDateClick(item)"
          >{{ item.label }}</AppTag
        >
      </div>
...
const ranges = [
  ...
  {
    label: "昨天",
    dates: [moment().subtract(1, "days"), moment().subtract(1, "days")]
  },
  {
    label: "前天",
    dates: [moment().subtract(2, "days"), moment().subtract(2, "days")]
  },
...
const compareRanges = [
  ...
  {
    label: "大前天",
    dates: [moment().subtract(3, "days"), moment().subtract(3, "days")]
  },
  {
    label: "上3天",
    dates: [moment().subtract(5, "d"), moment().subtract(3, "d")]
  },
  {
    label: "上7天",
    dates: [moment().subtract(13, "d"), moment().subtract(7, "d")]
  },
  {
    label: "上14天",
    dates: [moment().subtract(27, "days"), moment().subtract(14, "days")]
  },
  {
    label: "上30天",
    dates: [moment().subtract(59, "days"), moment().subtract(30, "days")]
  },
  {
    label: "上60天",
    dates: [moment().subtract(119, "days"), moment().subtract(60, "days")]
  },
  {
    label: "上90天",
    dates: [moment().subtract(179, "days"), moment().subtract(90, "days")]
  },
  {
    label: "上周",
    dates: [
...
export default {
  ...
  data() {
    return {
      ...
      momentFormat: {
        ...
        stringify: date => {
          return date
            ? moment(date).format(this.options.format || "YYYY-MM-DD")
            : "";
        },
        // [optional]  String to Date
        parse: value => {
          return value
            ? moment(value, this.options.format || "YYYY-MM-DD").toDate()
            : null;
        }
      }
    };
  ...
  methods: {
     ...
     */
    quickDateClick(item) {
      this.dates = this.options.range
        ? item.dates.map(v => v.format(this.options.valueType || "YYYY-MM-DD"))
        : item.dates.format(this.options.valueType || "YYYY-MM-DD");
