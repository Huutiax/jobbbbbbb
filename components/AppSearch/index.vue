<template>
  <el-card
    class="search-filter app-card"
    :class="{ 'search-filter--not-block': !blockButton }"
  >
    ...
    <el-form :model="form" class="app-form" :label-width="labelWidth">
      <el-row :gutter="20">
        ...
        >
          <el-form-item
            v-show="i < defaultColsNumber || !isCollapsed"
            :label="item.label"
    ...
    <div v-if="blockButton" class="text-center base-box--bottom">
      ...
      <el-button
        v-if="options.length > defaultColsNumber"
        type="text"
        @click="handlecollapse"
      >
        {{ !isCollapsed ? "收缩" : "展开" }}
        <i
          :class="!isCollapsed ? 'el-icon-arrow-up' : 'el-icon-arrow-down'"
        ></i>
      </el-button>
    </div>
  </el-card>
...
export default {
      ...
      default: () => [
        {
          key: "keyword",
          label: "关键字",
          component: { name: "input", placeholder: "请输入关键字" }
        },
  ...
  methods: {
    ...
    handleClearSearch: debounce(
      function() {
        ...
        Object.keys(this.form).forEach(key => {
          this.form[key] =
            this.options.filter(v => v.key === key)[0].component.name ===
            "range"
              ? []
              : "";
        });
    ...
    /**
     * @create 2021/09/01 14:21
     * @creator 潜
     * @description 处理折叠与展开 方法
     */
    handlecollapse() {
